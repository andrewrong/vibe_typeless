...
4. 保留原文的问题形式，不回答
5. 术语保护（非常重要）：
   - 金融术语：行权价、看涨期权、看跌期权、做多、做空、止损、止盈等
   - 技术术语：Docker, Kubernetes, Python, React, IBKR 等
   - 必须保留原文提及的所有专业术语，禁止翻译或改写
...
- 输出必须是纯文本，不含任何格式标记
- 绝对禁止输出任何与输入文本无关的内容
- **输出必须使用简体中文**：如果输入包含繁体中文，必须转换为简体中文输出

## 分段规则（遵守但不过度）
- 仅当输入文本明确包含多个话题或列表时才分段
- 短句（少于20字）通常不需要分段
- 不要为单句输入添加额外的换行
